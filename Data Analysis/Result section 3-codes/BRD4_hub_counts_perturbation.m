%% BRD4 Condensate Size — Box Plot + t-tests vs Ctrl
...
% ============================================================
filename  = '/Users/janaa/Desktop/MS2 transcription/Condensate_counts_all 6-22.xlsx';   % <-- your Excel file
sheetName = 'Sheet1';                              % sheet index or name, e.g. 'Sheet1'

% Column range for each condition (rows 5-500)
ranges = {'B5:B500', 'E5:E500', 'H5:H500', 'L5:L500', ...
          'P5:P500', 'T5:T500', 'X5:X500'};
labels = {'Ctrl', 'JQ1', 'iP300', 'iBRG1', ...
          'MED14 depl.', 'RAD21 depl.', 'del OCT4/SOX2'};

% --- Pick which groups to include (keep Ctrl as index 1, first) ---
pick   = [1 7];          
ranges = ranges(pick);
labels = labels(pick);

nGroups = numel(labels);   % your existing line below already does this
