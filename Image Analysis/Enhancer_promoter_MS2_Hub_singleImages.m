% % % % Enhancer-Promoter-MS2-BRD4 hub_configuration_PIPELINE.m
...
% % %    Step 5 : Nucleus segmentation using Cellpose
% % %    Step 6 : Proximal condensate/Hub peak detection in enhancer local neighborhood
% % %    Step 7 : Primary hub wrt enhancer/promoter 3D co-ordinates 
% % %    Step 8 : Enhancer-Promoter/Enhancer-Hub/Promoter-Hub 3D distance calculations
% % %    Step 9:  Save .mat
% % %
% % %  Dependencies on path:
% % %  ReadImage6D2, SNR_inc2, track_spot2D, single_zstack, fit_Gaussian3D, maskavg, HMM_fit_fun, binary, condensate_search_v9, point_location2D
...
C_in =[
1723.162939	1479.13738
1708.146965	423.5463259
1210.28754	342.5239617
2137.699681	390.543131];   % Approximate pixel co-ordinates of enhancer (Verify the cell has all 4 channel labels in ImageJ)
...
save_filename='Condensate parameters BRD4 4 color ctrl cell 3 1-29-2026 3x thr';
Input_zstack='/Volumes/My Passport/MCP BFP 2nd sort 1-29-2026/4 color ctrl cell 3_SIM.czi';
MIP_filename='/Volumes/My Passport/MCP BFP 2nd sort 1-29-2026/4 color ctrl cell 3_SIM_Maximum intensity projection.czi';
...
R=30;  %tracking radius for enhancer
R_p = 30; %search radius for promoter
R_m = 30; %search radius for MS2

R_fit = 10;  % 3d gaussian fitting radius
fit_thr = 0.1; % minimum R2 value needed for a good gaussian fit from tracked spot
...
for j=1:vis
    ...
    C_cent_p(start_f,1,j)=C_in(j,1);
    C_cent_p(start_f,2,j)=C_in(j,2);
    C_cent_p(start_f,3,j)= round(z_slice/2);
end
...
%%=======more useful for single large field of views ========
...
for j=1:vis
    ...
    inputImage_cond1 = inputImage_cond().*seg_OUT{1,1}{id,1};   % use that specific nuclear mask

    inputImage_cond2 = SNR_inc2(inputImage_cond1,0.1);    % BACKGROUND REDUCTION AND SIGNAL BOOSTING-

    BW = imbinarize(inputImage_cond1,mult*t_cond);    % binarize to get rough estimate of condensates

    BW2 = bwareafilt(BW,[10 inf]);   % filtering very small condensates from this rough binary image

    BW3 = inputImage_cond().*BW2;

    else 
        BW = imbinarize(inputImage_cond,mult/2*nuc_ave);
...
R_search_c  = 10; %1st search radius is 5 pixel or ~ 150 nm 
...
prom_xyz = (reshape(prom_xyz,3,[]))';
TSS_xyz = (reshape(TSS_xyz,3,[]))';
MS2_score = (reshape(MS2_score,5,[]))';
Cond_xyz1 = (reshape(Cond_xyz1,3,[]))';
Cond_xyz2 = (reshape(Cond_xyz2,3,[]))';
E_C_dist1 = (reshape(E_C_dist1,2,[]))';
E_C_dist2 = (reshape(E_C_dist2,2,[]))';
P_C_dist1 = (reshape(P_C_dist1,2,[]))';
P_C_dist2 = (reshape(P_C_dist2,2,[]))';
E_P_dist_3d = (reshape(E_P_dist_3d,2,[]))';
E_P_dist_2d = (reshape(E_P_dist_2d,2,[]))';
% % %=======%=======%=======%=======%=======
